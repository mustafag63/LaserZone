...
const create = async (req, res) => {
  try {
    ...
    // If group is already full (no open spots), start it as closed
    if (parsedLeaderPlayerCount === parsedPartySize) {
      // closed group — still create it so the leader can track it
    }
...
const update = async (req, res) => {
  try {
    ...
    if (!name || !date || !time || partySize === undefined) {
      return res.status(400).json({ message: 'name, date, time and partySize are required.' });
    }
    ...
    const group = await GroupReservation.findById(id);
    if (!group) return res.status(404).json({ message: 'Group not found.' });
    if (group.leaderUserId !== req.user.id) return res.status(403).json({ message: 'Only the group leader can edit this group.' });

    if (parsedPartySize < group.currentCount) {
      return res.status(400).json({ message: `Party size cannot be less than current member count (${group.currentCount}).` });
    }

    const updated = await GroupReservation.update(id, req.user.id, {
      name: name.trim(),
      date,
      startTime: time,
      partySize: parsedPartySize,
